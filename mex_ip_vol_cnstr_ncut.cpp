...
void mexFunction(int nlhs, mxArray *plhs[],
  ...
  if (nrhs != 8 || nlhs != 8) {
    mexWarnMsgTxt("Usage: [X,alpha,PrimalObj,DualObj,iter,Xbest,PrimalObjBest,DualObjBest]=mex_ip_vol_cnstr_ncut(W,Y,alpha,MAXITER,EPS,MaxSumSquaredWeights,MAXITER_temp,debug)");
    return;
  }
...
//  int MAXITER_temp=50;
  ...
  while(iter<MAXITER){
	...
	if (iter==MAXITER-1 || iter==MAXITER_temp || iter<10)     {
  ...
  //          Dproj2[i]=(D[i]-Dproj2[i]);//*eta;
   ...
   //
       ...
       /* max_f=0;
        ...
        primalVal_orig=primalVal_orig+primalVal2;
