function [lambda,indvec2,indvec1]=compute_lambda(f,gamma,k1prime,k2prime,deg_rest,gvolJ,degJ,assocJ,wval_rest,ix_rest,jx_rest,gdeg_rest)
% Computes the objective of the constrained maximum density problem with
% subset and cardinality constraints
%
% Usage: [lambda,indvec,indvec1]= compute_Lambda(f,gamma,num,k1,k2,deg,wval,ix,jx,gdeg,ind_rest,subset,gvolJ)
%
% Input: 
% f         The current vector
% gamma     The penalty parameter gamma
...
%
    ...
    [~,sortind]=sort(f);
    num=length(f);
    indvec1=zeros(num,1);indvec1(sortind(num-k1prime+1:num))=1;
    indvec2=zeros(num,1);indvec2(sortind(num-k2prime+1:num))=1;
    

    R1 = f'*gdeg_rest+ gamma*f'* ones(length(f),1) +   (gvolJ + gamma * k1prime)*max(f);
    R2= gamma*f'* (indvec1 + indvec2);
