function [dc_best, maxdens_best, gam_best, dc_all,maxdens_all,gam_all] ...
    = cnstr_maxdens_subset(W, k1,k2,gdeg,subset,numRuns)
...
% 
    ...
    while (gamma_loop)
        ...
        for i=1:numRuns
            ...
            [dc,maxdens,lambda]= cnstr_maxdens_subset_single_run(W,k1,k2,gdeg,start,subset,gam);
            
            feasible=sum(dc>0)<=k2 && sum(dc>0)>=k1;
            
             %store result if feasible and better
            if(feasible)
                % we store the best feasible result for each starting point
