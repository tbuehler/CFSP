...
% subset constraint. The subset constraint as well as the volume constraint 
% are treated as soft constraints which are controlled by corresponding 
% parameters gamma_sub and gamma_vol. The optimization problem is solved for 
% all provided values of gamma_sub and gamma_vol using the method RatioDCA.
%
% Corresponding paper:
...
%
% Usage:  [cluster_grid, ncut_grid, vol_grid, subs_grid] = soft_constrained_ncut(...
%         W, k, h, subset, gammas_subs, gammas_vol, starts, verbosity);
%
% Input:
...
%
% Output:
% cluster_grid      Grid containing obtained clusterings for all values of gamma1 and gamma2
...
function [clusters, ncut, feasible, lambda] = ...
    ...
    [clusters, ncut, feasible, lambda] = vol_cnstr_ncut_subset_penalty(W, ...
                        k, h, start, subset, gamma1*scaling, gamma2, verbosity);
        ...
        for p=1:length(pert_probs) % try different perturbation probabilities 
            ...
            while pert_counter < 1 % try until a perturbation does not improve the result
                ...
                [cluster_pert, ncut_pert, feasible_pert, lambda_pert] = ...
                  vol_cnstr_ncut_subset_penalty(W, k, h, pert_start, subset, gamma1, gamma2, verbosity); 
                if (verbosity>1)
